...
#. a `PaintAgree houseOwner painter refNo` contract has both the
   house owner and the painter as signatories.
...
**Exercise** actions on the contract allowed by the contract model, and
...
the exercise actions the party can perform according to the authorization 

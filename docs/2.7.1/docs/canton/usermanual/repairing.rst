...
- Reassigning contracts from one sync domain to another
...
.. note::
    Please note that the given section describes a preview feature.
...
- copy over the agreed-upon set of contracts to a brand new sync domain,
...
- the sync domain "newDomain" that we will rely on as a replacement sync domain, and
- the "enable-preview-commands" configuration needed to make available the "repair.change_domain"
  command.

In practice you would only add the new sync sync domain once you have the need to recover from domain loss and also only then
...
the point that the participants no longer have access to any state from that sync domain.
...
- and invoke "repair.change_domain" (enabled via the "features.enable-preview-commands" configuration) in order to
  assign the contracts to the new sync domain.
...
Starting with ``--manual-start`` will prevent the participant to attempt to reconnect to the sync domains.

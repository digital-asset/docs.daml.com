...
Manage Synchronizer Entities
============================

.. _domain_bootstrapping:

Setting up a Distributed Synchronizer With a Single Console
-----------------------------------------------------------

If you're running a synchronizer in its default configuration as shown previously,
it will have a sequence and mediator embedded and these components will be automatically bootstrapped for you.

If your synchronizer operates with external sequencers and mediators,
you will need to configure a synchronizer manager node (which only runs topology management) and bootstrap your synchronizer
...
The synchronizer manager also needs its identity to be generated and ready before we can bootstrap the synchronizer:
...
.. note::
    ...
    This is technically only required when accessing the synchronizer manager through a remote console, but is a good practice regardless.

Now you can initialize the distributed synchronizer as follows:
...
At this point a participant should be able to connect to a sequencer and operate on that synchronizer:

.. snippet:: distributed_domain_install
    .. success:: participant1.domains.connect_local(sequencer1)
    .. success:: participant1.health.ping(participant1)

Synchronizer managers are configured as ``domain-managers`` under the ``canton`` configuration.
Synchronizer managers are configured similarly to synchronizer nodes, except that there are no sequencer, mediator, public API,
...
You can either initialize such sequencers as part of the initial synchronizer bootstrap shown above or
...
Setting up a Distributed Synchronizer With Separate Consoles
------------------------------------------------------------
...
.. note::
    ...
    Please ensure that all of the nodes in the distributed synchronizer are started before proceeding.


Initially the synchronizer manager must transmit its synchronizer parameters from its console by saving the parameters to a file.
The synchronizer ID, serialized as a string, must also be transmitted.
...
The synchronizer manager must then authorize the sequencer's key. Optionally, repeat this for any extra sequencer keys.
...
The synchronizer manager must now authorize the mediator's key and also authorize the mediator to act as part of this synchronizer.
...
After that, still on the synchronizer manager's console, the synchronizer manager must collect the list of topology transactions, which includes all the key authorizations and
a few other things it needs to broadcast to all synchronizer members. This is now saved to a file.
...
The sequencer then reads this set of initial topology transactions and sequences it as the first message to be sequenced in this synchronizer.
This will allow the synchronizer members whose keys were authorized in previous steps to connect to this sequencer and operate with it.
...
To initialize the mediator, it will need a connection to the sequencer and the synchronizer parameters.
Optionally, repeat this for any extra mediator nodes.
...
The synchronizer manager will also need a connection to the sequencer in order to complete its initialization.
...
At this point the distributed synchronizer should be completely initialized and a participant should be able to operate on this synchronizer by connection to the sequencer.
...
Adding new sequencers to a distributed synchronizer
---------------------------------------------------

For non-database-based sequencers such as Ethereum or Fabric sequencers, you can either initialize them as part of the
regular :ref:`distributed synchronizer bootstrapping process <domain_bootstrapping>` or dynamically add a new sequencer at a later point as follows:
...
Similarly to :ref:`initializing a distributed synchronizer with separate consoles <domain_bootstrapping_separate_consoles>`,

...
coupons are pre-defined using a fixed annualized rate. In contrast, the coupons of a
floating rate bond depend on a reference rate for each future coupon period. Hence,
 previous lifecycle instruments with pre-defined payments, e.g. a fixed rate

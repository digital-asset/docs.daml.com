...
.. literalinclude:: ../../daml/code-snippets/Batching.daml
  :language: daml
...
To batch, group the operations on the ledger based solely on the lists of informees and confirming
parties. You can use choice observers to align informees if necessary. With the previous model as reference, the batch
function can only reduce the total number of views if the observer of the aggregating choice is the

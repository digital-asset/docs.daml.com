...
.. note:: These metrics are disabled by default to avoid a negative performance impact under high load scenarios.
    They can be turned back on by setting the following configuration value: ``canton.monitoring.metrics.report-execution-context-metrics = true``

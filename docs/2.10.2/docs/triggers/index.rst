...
on. In this case, we specify ``AllTemplates`` which means that
the trigger will receive events for all template and interface types defined in the
DAR.

.. note::
 
   The templates specification ``AllInDar`` should no longer be used,
   as it installs triggers defined in all DAR files known to the service.

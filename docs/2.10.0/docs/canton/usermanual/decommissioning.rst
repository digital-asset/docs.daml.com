...
#. Disable the participant to be decommissioned in all sequencers to remove any sequencer data associated with it;
   this can be done through the :ref:`sequencer.disable_member <sequencer.disable_member>` command.
...
#. Among those keys, find the ones not shared by other sequencers by issuing the :ref:`keys.secret.list <keys.secret.list>` command

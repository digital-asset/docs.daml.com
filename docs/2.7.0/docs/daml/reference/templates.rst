...
.. warning::
   Some uses of the ``this`` keyword in template-local definitions can cause an infinite loop of evaluation as a circular dependency arises. Hence any usage of: 
   - ``key this``
   - ``observer this``
   - ``signatory this``
   - interface methods
   in a template-local let definition will result in an error generated by the inifinate loop.

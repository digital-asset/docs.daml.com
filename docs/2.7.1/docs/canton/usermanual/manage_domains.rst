...
virtual shared ledger. However, the network itself is made up of synchronization (sync) domains that are used by participants to run the Canton
...
We can trigger that handshake again by attempting to reconnect to the sync domain again:
...
while the sync domain is running); these are called **dynamic synchronization (sync) domain parameters**. When the sync domain is bootstrapped, default
...
the grpc channel size on the sequencer node and the maximum size that a sequencer client is allowed to reassign.
...
On a non-distributed deployment, this means modifying both the sync domain and the participants configuration as follows:

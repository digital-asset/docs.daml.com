...
are explained in the :ref:`architecture section <identity-manager-1>`, while this writeup is meant to give a high
level explanation.

The identity management system is self-contained and built without a trusted central entity or pre-defined root
certificate such that anyone can connect with anyone, without the need for central approval and the
...
The system entities require knowledge about the keys used for encryption and signing by the
...
~~~~~~~~~~~~~~~~~~~~~
In order to remain flexible and be able to change keys and cryptographic algorithms, we do not identify the
...
impact. They can create new keys, new namespaces, and the identity of new participants, parties and even sync domains. And
they can export these topology transactions such that they can be imported at another topology manager. This allows you to
...
two companies. Therefore, Canton provides an administrative command which allows you to associate a randomized
...
..  note::
    ...
    we recommend you build a corresponding Daml workflow and some automation logic, listening to the
    results of the Daml workflow and updating the display name accordingly.

Life of a Party
~~~~~~~~~~~~~~~
In the tutorials, we use the ``participant.parties.enable("name")`` function to set up a party on a participant.
...
~~~~~~~~~~~~~~~~~~~~~~
Key to supporting topological flexibility is that participants can easily be added to new sync domains. Therefore, the
...
5. The participant will download and verify the sync domain ID from the sync domain. The :ref:`domain id <bootstrapping-idm>`
   ...
   ``domains.register`` call, the participant will use and store the one downloaded. We assume here that the sync domain id is
...
6. The participant downloads the *static sync domain parameters*, which are the parameters used for the transaction protocol
   on the particular sync domain, such as the cryptographic keys supported on this sync domain.
...
8. The request service forwards the transactions to the sync domain topology manager, who attempts to add it to
...
10. Once the participant is properly enabled on the sync domain and its signing key is known, the participant can subscribe
    to the ``SequencerService`` with its identity. In order to do that and in order to verify the authorization of any
...
~~~~~~~~~~~~~~~~~~~~~~
The default initialization behavior of participant and sync domain nodes is to run their own topology manager. This provides
...
~~~~~~~~~~~~~~~~~~~~
As explained, Canton nodes auto-initialize themselves by default, running their own topology managers. This is
...
First, we create three parties to use in subsequent examples:
...
Turn off transaction processing on the sync domain by setting the rate to 0 and wait for all timeouts to have
...
The target participant must then be disconnected from the sync domain before it can import data:
...
When importing is finished, reconnect to the sync domain using:
...
Before we continue, we need to ensure that the target participant is now disconnected from the affected sync domains, in order to
avoid the target participant receiving transactions for the new party prior to the complete transfer of the
active contract store. Therefore, we disconnect the participant from all sync domains:
...
damage their system by accident due to forgetting to disconnect from the sync domain.
...
looking at the topology store of that particular sync domain connection:
...
Note that you need to do this for every sync domain separately with the correct timestamp of the activation of the party.
In our example, there is only one sync domain.
...
Once the entire active contract store has been imported, the target participant can reconnect to the sync domain:
...
In the following, we demonstrate the basic steps how to initialize a node:

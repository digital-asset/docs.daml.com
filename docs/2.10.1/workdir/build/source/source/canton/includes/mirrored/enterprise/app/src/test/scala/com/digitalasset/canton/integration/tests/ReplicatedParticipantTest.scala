...
import com.digitalasset.canton.participant.sync.SyncServiceError.SyncServiceDomainBecamePassive
import com.digitalasset.canton.participant.sync.SyncServiceInjectionError.PassiveReplica
import org.scalatest.Assertion

import scala.concurrent.duration.*

trait ReplicatedNodeHelper { self: EnterpriseIntegrationTest =>
  ...
  protected def waitPassive(participant: ParticipantReference): Unit =
    checkStatusActive(
      participant,
      active => if (!active) succeed else fail(s"Participant ${participant.name} is not passive"),
    )

}

trait ReplicatedParticipantTestSetup extends ReplicatedNodeHelper {
  self: EnterpriseIntegrationTest & EnterpriseEnvironmentSetup =>

  protected def setupPlugins(
      storagePlugin: EnterpriseSetupPlugin,
      additionalPlugins: Seq[EnterpriseSetupPlugin] = Seq.empty,
  ): Unit = {
    ...
    registerPlugin(
      new UseConfigTransforms(
        Seq(
          ConfigTransforms.setParticipantMaxConnections(16)
        ),
        loggerFactory,
      )
    )
  ...
  protected def failPassivePing(
    ...
    val expectedPingServiceFailureLogs = List(
      "GrpcServiceUnavailable: UNIMPLEMENTED/Method not found: com.digitalasset.canton.participant.admin.v0.PingService/ping",
      "sending response PingResponse(Failure(PingFailure())",
      "Failed(Completion(Status(UNAVAILABLE, Command rejected, as the participant is shutting down.)",
    )
    ...
    val expectedWarnsAfterFailover: List[String] =
      if (afterFailure)
        List(
          "Creating a new write lock on new connection",
          "Synchronous event processing stopped because instance became passive",
          "Database connection was lost, trying to rebuild",
          "Failed to acquire lock on passive instance",
          "Closing resilient sequencer subscription because instance became passive",
          SyncServiceDomainBecamePassive.id,
    ...
    loggerFactory.assertThrowsAndLogsSeq[CommandExecutionFailedException](
  ...
  protected def setupReplicas(
    ...
    def startParticipant(name: String): ParticipantReference = {
      ...
      ref.health.wait_for_running()
      ref.health.wait_for_initialized()
    ...
    activeParticipant.domains.connect_local(da)
    waitActive(activeParticipant)
    ...
    // start a non-replicated participant and connect to the domain
    assert(!Seq(activeParticipant, passiveParticipant).contains(participant3))
    participant3.start()
    participant3.domains.connect_local(da)
  }

  protected lazy val baseEnvironmentDefinition: EnterpriseEnvironmentDefinition =
    EnterpriseEnvironmentDefinition.singleDomainWithManyParticipants
...
/** Testing a replicated participant node without crashing the nodes, but only with a graceful shutdown. */
trait ReplicatedParticipantTest
  ...
  "A replicated participant" must {
    ...
    "set passive on non-replicated participant should recover" in { implicit env =>
      ...
      participant3.replication.set_passive() shouldBe false
  ...
  override def testFinished(env: EnterpriseTestConsoleEnvironment): Unit =

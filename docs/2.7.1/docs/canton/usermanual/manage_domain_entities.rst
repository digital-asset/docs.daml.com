...
If you're running a sync domain node in its default configuration as shown previously in this current page,
...
Initially the sync domain manager must transmit its syn domain parameters from its console by saving the parameters to a file.
...
After that, still on the sync domain manager's console, the sync domain manager must collect the list of topology transactions, which include all the key authorizations and
...
Additionally, please note that if more than one sequencer has been initialized, any mediator node and sync domain manager can choose

...
* **Evidence**. For any data which I receive from the IPS I can get the set of
  ...
  associated evidence contains a descriptor that I can use to read up in the
...
Design Principles
=================
To understand the approach, a few key principles need to be
introduced.

The synchronization protocol is separated from the topology protocol. However,
to leverage the composability properties of the synchronization
...
Definitions
-----------
To construct a global composable topology system that incorporates identity, we introduce
...
For the sake of brevity, we omit the identity providing service signature
...
where the `?` is a placeholder for a specific permissioning level. The
...
* **namespace delegations**: `\{+/-; (*;I_{k}) \Rightarrow p_{l}\}^{[\tilde{s}_{k}]}`
  ...
  then the key can do everything on that namespace, except issuing `NamespaceDelegation`.
...
local effect. This means that the removal of a root key `\{-, p_{k}\}` does
not invalidate all transactions that have been signed previously by the key,
directly or indirectly. To revoke a key as in "invalidating everything
...
Default Party
^^^^^^^^^^^^^
Given that `(N,I_{k})` and `(P,I_{k})` are both unique identifiers used to refer to participants and parties, we can introduce for every participant
...
submit transactions in the name of the party. This behavior can be detected by any other participant hosting the
party, but these participants cannot prevent the transaction from being accepted. Therefore, the distinction between
*submission* and *confirmation* permissions in the party-to-participant mappings is only guaranteed to be respected by
...
We include a service with the committer that allows us to query the data once the
left side has been learned. This means that once `X` of `H(X)` is known, a
...
participating bank. Second, we ensure that nobody can know how many
...
Clearly, in such a case, the transfer must fail in a predictable manner.
Therefore, we introduce two new rules

  1) transfer-out on `D_{S}` will be rejected if `(P \rightarrow
...
High-Level Picture
==================
The following illustration provides a high-level overview of the identity management architecture and flows.
...
* **Message Forwarder** - The topology state requires that the topology transactions be applied in the
